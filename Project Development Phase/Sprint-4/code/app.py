...
app.config['curr_email']='a@gmail.com'
app.config['curr_name']='Ralph'
app.config['curr_filename'] = ''
...
@app.route("/use_another",methods=['GET','POST'])
def use_another():
  ...
  if request.method == 'POST':
    ...
    if(isUser['PASSWORD']==password):
        if str(isUser['VERIFIED']) == '1':
            ...
            app.config['curr_email']=email
            app.config['curr_name']=isUser['FULL_NAME']
            return render_template('upload_image.html')
        else:
            return render_template('verify.html', email = email)
    else:
...
def send_email_report(username,email, result, confidence):
    print(username, email, result, confidence)
    msg = Message('Skin Reoort ' + username , sender =   'baap11102@gmail.com', recipients = [email])
    msg.body = "Hello, "+username+" Your Skin Disease report :  "+ result+' with confidence: '+ confidence
    mail.send(msg)
...
def download():
    return send_from_directory(os.path.join(app.root_path, 'static','uploaded_images'),app.config['curr_filename'] , as_attachment=True);
...
def predict():
    if request.method == 'POST':
        ...
        app.config['curr_filename'] = file_name
        if len(result_df.index)==0:
            result ='No disease, you\'re absolutely fit & fine '
            confidence = 'N/A' 
            send_email_report(app.config['curr_name'],app.config['curr_email'],result,confidence)
            return render_template('predict.html', result=result,file_path=destination,confidence='N/A')
